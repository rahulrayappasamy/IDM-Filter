  ...
  const handleProcessData = () => {
    ...
    for (let maxCount = 1; maxCount <= 10; maxCount++) {
      ...
      parsedData.forEach((row) => {
        ...
        if (jobLevelsArray.length && jobLevel !== null && !jobLevelsArray.includes(jobLevel)) return;
        if (!dict[company]) {
          dict[company] = 1;
          count++;
    ...
    const userMaxCount = parseInt(prompt(`Output Counts (Contacts per account 1 to 10):\n${Object.entries(outputCounts)
    ...
    const processedData = parsedData.filter((row) => {
      const { company, jobLevel } = row;
      if (jobLevelsArray.length && jobLevel !== null && !jobLevelsArray.includes(jobLevel)) return false;
...
const copyCompIDsToClipboard = (chunkData) => {
  ...
  const copyIndivIDsToClipboard = (chunkData) => {
    const indivIDs = chunkData.map(row => row.personID).join('\n');
    navigator.clipboard.writeText(indivIDs);
  };
  ...
  const handleExportToExcel = () => {
    ...
    outputData.forEach((row) => {
      rows.push([row.company, row.personID, row.jobLevel !== null ? row.jobLevel : '']);
    });
  ...
  return (
    <div>
      ...
      <div className="joblev">
        <label htmlFor="jobLevels">Job Levels (optional, comma-separated):</label>
        <input 
      ...
      <div className="single">
        <p style={{ marginTop: '20px', fontSize: '16px' }}>
          Enter the IDM COMP and IDM Person ID:-
        </p>
      </div>
    </div>
    ...
    <div className="form-floating">
      ...
      <div className='bt'>
      <button className='btn btn-primary bnw' onClick={handleProcessData}>Process Data</button>
      <button className='btn btn-primary bmw' style={{ marginLeft: '20px' }} onClick={handleReset}>Reset</button>
      </div>
      
      <div>
        <h2 style={{marginLeft:'15px',marginBottom:'20px',marginTop:'15px'}}>Processed Data</h2>
      </div>
      {outputData.length > 0 && (
        <div style={{marginBottom:'20px'}}>
          <button className='btn btn-secondary btg' onClick={handleExportToExcel}>Export to Excel</button>
          {outputData.length > 3000 && <button className='btn btn-secondary btg' onClick={handleSplitData}>Split Data</button>}
        </div>
      )}
      {/* Render Split Data */}
  ...
  splitDataChunks.map((chunk, index) => {
    ...
    return (
      ...
      >
        <h4 style={{ paddingTop: '15px' }}>Chunk {index + 1} (3K)</h4>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <button
            className="btn btn-success"
            onClick={() => copyCompIDsToClipboard(chunk)}
          >
            Copy Comp_IDs to Clipboard
          </button>
          <button
            className="btn btn-success"
            onClick={() => copyIndivIDsToClipboard(chunk)}
          >
            Copy Indiv_IDs to Clipboard
          </button>
